...
flag_geminal=0
try:
  CCH_file=open('./'+str(sys.argv[5]),'r')
...
for i in range(len(SEQ)):
  if (SEQ[i]=='A' and SEQ[i] in labeling):
...
for i in range(peak_type.shape[0]):
  ...
  if tot==0:pass
   ...
   for j in range(6):
    if ((((j==1 or j==2 or j==3 or j==4) and (peak_type[i,j]/float(tot))>=0.2) or
  ...
  if text=='':text='!!!!!OUTLINER!!!!!'
  else:pass
  spaces=len('  '+HMQC[i].split()[0])
  HMQC_newfile.write('  '+HMQC[i].split()[0]+(14-spaces)*' '+HMQC[i].split()[1]+'   '+HMQC[i].split()[2]+'\t\t'+text+'\n')
